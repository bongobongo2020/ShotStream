...
import torch
# import wandb # MODIFIED: Removed wandb
...
class Trainer:
    ...
    def __init__(self, config):
        ...
        # ------------------------------------- One Logger Setup ----------------------------------------------
        if self.use_one_logger and dist.get_rank() == 0 and not self.disable_tensorboard:
    ...
    def train(self):
        ...
        try:
            while True:
                ...
                # Logging
                # MODIFIED: Replaced wandb.log with self.writer.add_scalar
    ...
    def _visualize(self):
        ...
        for vid_len in self.vis_video_lengths:
            ...
            for idx, video_np in enumerate(videos):
                ...
                # Transpose from (T, H, W, C) to (T, C, H, W) for write_video
                write_video(out_path, video_tensor.permute(0, 3, 1, 2), fps=16)

                # MODIFIED: Log video to TensorBoard
                if self.is_main_process and self.writer:
                    # Add batch dimension and ensure correct shape (N, T, C, H, W)
                    video_tensor_for_tb = video_tensor.permute(0, 3, 1, 2).unsqueeze(0)
                    self.writer.add_video(f"Validation_Rank{dist.get_rank()}/{video_name}", video_tensor_for_tb, self.step, fps=16)
