...
# 允许 Inductor 在没有找到 Triton 配置时回退到 ATen 实现
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if is_tf:
                ...
                x = flex_attention(
                    query=padded_roped_query.transpose(2, 1),  # [b, n, head, c] --> [b, head, n, c]
                    key=padded_roped_key.transpose(2, 1),  # [b, n, head, c] --> [b, head, n, c]
                    value=padded_v.transpose(2, 1),
                    block_mask=block_mask  # [1, 1, n, n]
                )[:, :, :-padded_length].transpose(2, 1)
        else:
