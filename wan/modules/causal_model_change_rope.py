...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # [TODO]
        elif kv_cache is not None and kv_cache_context is None:  # for kv cache, without context cache
            frame_seqlen = math.prod(grid_sizes[0][1:]).item()
            current_start_frame = current_start // frame_seqlen
            roped_query = causal_rope_apply(
                q, grid_sizes, freqs, start_frame=current_start_frame).type_as(v)  # [b num_token head channel]
            ...
            condition_start_frame = kv_cache_context[0]['k'].shape[1] // frame_seqlen
            ...
            if self.local_attn_size != -1 and (current_end > kv_cache["global_end_index"].item()) and (
                ...
                print(f"There Might BUG in L403 in causal model, please fix it")
            else:
                # Assign new keys/values directly up to current_end
                local_end_index = kv_cache["local_end_index"].item() + current_end - kv_cache["global_end_index"].item()
                local_start_index = local_end_index - num_new_tokens
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        self_attn_result = self.self_attn(
            ...
            seq_lens, grid_sizes,
            freqs, block_mask, kv_cache, current_start, cache_start, kv_cache_context=kv_cache_context)
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    # @staticmethod
    # def _prepare_blockwise_causal_attn_mask_with_condition(
    #     device: torch.device | str, num_frames: int = 21,
    #     frame_seqlen: int = 1560, num_frame_per_block=1, local_attn_size=-1, condition_num_frames: int = None,
    # ) -> BlockMask:
    #     """
    #     we will divide the token sequence into the following format
    #     [1 latent frame] [1 latent frame] ... [1 latent frame]
    ...
    #     condition_length = condition_num_frames * frame_seqlen
    #     # frame_indices = torch.arange(
    #     #     start=0,
    #     #     end=total_length,
    #     #     step=frame_seqlen * num_frame_per_block,
    #     #     device=device
    #     # )
    #     ends[0: condition_length] = condition_length
    #     frame_indices = torch.arange(
    ...
    def _forward_train(
        ...
        #             )
        #         else:
        #             if causal_use_condition_mask:
        #                 self.block_mask = self._prepare_blockwise_causal_attn_mask_with_condition(
        #                 device, num_frames=x.shape[2],
        #                 frame_seqlen=x.shape[-2] * x.shape[-1] // (self.patch_size[1] * self.patch_size[2]),
        #                 num_frame_per_block=self.num_frame_per_block,
        #                 local_attn_size=self.local_attn_size,
        #                 condition_num_frames=condition_frame_number
        #             )
        #             else:
        ...
        for i in range(iter_num):
            ...
            end_index = start_index + (i+1) * self.num_frame_per_block * frame_seqlen
